...
C1survey <- read_csv("Class 1 Survey Fall 2025 2 (1).csv")
...
```
```{r}
# Task 7: What are the most common hobbies in the class?
C1_optD <- C1survey %>% mutate(hobby = str_squish(str_to_lower(hobby)))
hobby_counts <- C1_optD %>% filter(!is.na(hobby), hobby != "") %>% count(hobby, sort = TRUE)
head(hobby_counts, 5)
